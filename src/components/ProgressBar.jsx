...
const ProgressBar = ({ selectedStudent, competencyByStudent }) => {
    ...
    const coursesObject = Object.groupBy(competencyByStudent ?? [], ({ course_name }) => course_name);

    const coursesArray = Object.entries(coursesObject).flatMap(([courseName, courseData]) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const competentCredits = coursesArray?.reduce((sum, competency) => {
                return competency.status === "competent" ? sum + competency.competencyCreditValue : sum;
            }, 0);
